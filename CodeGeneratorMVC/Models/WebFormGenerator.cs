...
public class WebFormGenerator
{
    ...
    private string getPageLoadForEdit(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            ...
            strB.AppendLine(Space(tab.XXX) + "Dim myObject As " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized + " = get" + pClass.Name.Capitalized + "FromQueryString()");
        ...
        else
        {
            ...
            strB.AppendLine(Space(tab.XXX) + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized
    ...
    private string getGetFunctionForQueryString(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            strB.AppendLine(Space(tab.X) + "Private Function get" + pClass.Name.Capitalized + "FromQueryString() As " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized);
            strB.AppendLine(Space(tab.XX) + "Return " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.DALClassVariable.Name + ".get" + pClass.Name.Capitalized + "(Request.QueryString(\"id\"), True)");
            strB.AppendLine(Space(tab.X) + "End Function");
        }
        else
        {
            strB.AppendLine(Space(tab.X) + "private " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized
                      + " get" + pClass.Name.Capitalized + "FromQueryString()");
            strB.AppendLine(Space(tab.X) + "{");
            strB.AppendLine(Space(tab.XX) + "return " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.DALClassVariable.Name
    ...
    private string getFillFormForEdit(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            strB.AppendLine(Space(tab.X) + "Private Sub fillForm(ByVal my" + pClass.Name.Capitalized + " As "
+ pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized + ")");
        ...
        {
            strB.AppendLine(Space(tab.X) + "private void fillForm(" + pClass.NameSpaceVariable.NameBasedOnID
        ...
        strB.AppendLine(Space(tab.XXX) + "litTitle.Text = AliasGroup.Add.Capitalized " + conCat
        ...
        strB.AppendLine(Space(tab.XXX) + "litTitle.Text = AliasGroup.Edit.Capitalized " + conCat
        ...
        foreach (ClassVariable classVar in pClass.ClassVariables)
        {
            ...
            if (classVar.IsTextBox)
            {
                ...
                strB.AppendLine(Space(tab.XXX) + classVar.DefaultHTMLName + ".Checked = my" + pClass.Name.Capitalized + "." + classVar.Name + lineEnd);
            else if (classVar.IsDate)
            {
                strB.AppendLine(Space(tab.XXX) + classVar.GetMonthTextControlName + ".Text = my" + pClass.Name.Capitalized + "." + classVar.Name + ".Month.ToString()" + lineEnd);
                strB.AppendLine(Space(tab.XXX) + classVar.getDayTextControlName + ".Text = my" + pClass.Name.Capitalized + "." + classVar.Name + ".Day.ToString()" + lineEnd);
                strB.AppendLine(Space(tab.XXX) + classVar.getYearTextControlName + ".Text = my" + pClass.Name.Capitalized + "." + classVar.Name + ".Year.ToString()" + lineEnd);
            }
            else if (classVar.IsDropDownList)
            {
                NameAlias tempAlias = new NameAlias(classVar.ParameterType.Name.ToLower());
                if (lang == language.VisualBasic)
                    strB.AppendLine(Space(tab.XXX) + "For Each tempObject As " + pClass.NameSpaceVariable.NameBasedOnID + "." + classVar.ParameterType.Name + " In " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.DALClassVariable.Name + ".Get" + tempAlias.PluralAndCapitalized + "()");
                else
                {
                    strB.AppendLine(Space(tab.XXX) + "foreach (" + pClass.NameSpaceVariable.NameBasedOnID + "." + classVar.ParameterType.Name + " tempObject in " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.DALClassVariable.Name + ".Get" + tempAlias.PluralAndCapitalized + "())");
                    strB.AppendLine(Space(tab.XXX) + "{");
                }
                ...
                if (lang == language.VisualBasic)
                    ...
                    strB.AppendLine(Space(tab.XXX) + "}");
                strB.AppendLine(Space(tab.XXX) + classVar.DefaultHTMLName + ".SelectedValue = my" + pClass.Name.Capitalized + "." + classVar.Name + "ID.ToString()" + lineEnd);
            }
        }
        if (lang == language.VisualBasic)
        {
    ...
    private string getSaveChanges(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            ...
            strB.AppendLine(Space(tab.XX) + "Dim my" + pClass.Name.Capitalized + " As " + pClass.NameSpaceVariable.Name + "."
        ...
        else
        {
            ...
            strB.AppendLine(Space(tab.XX) + pClass.NameSpaceVariable.Name + "." + pClass.Name.Capitalized + " my" + pClass.Name.Capitalized
                            + " = get" + pClass.Name.Capitalized + "FromQueryString();");
        }
        bool doubleExists = false;
        bool integerExists = false;
        ...
        foreach (ClassVariable classVar in pClass.ClassVariables)
        {
            ...
            else if (classVar.IsDate)
            {
                if (!dateExists)
                    strB.AppendLine(Space(tab.XX) + IIf(lang == language.VisualBasic, "Dim tempDateTime As DateTime", "DateTime tempDateTime;").ToString());
                strB.AppendLine(Space(tab.XX) + "DateTime.TryParse(" + classVar.GetMonthTextControlName() + ".Text.Trim() & \"/\" & " + classVar.getDayTextControlName() + ".Text.Trim() & \"/\" & " + classVar.getYearTextControlName() + ".Text.Trim(), tempDateTime)" + lineEnd);
                strB.AppendLine(Space(tab.XX) + "my" + pClass.Name.Capitalized + "." + classVar.Name + " = tempDateTime" + lineEnd);
                dateExists = true;
            }
        }
        strB.AppendLine(Space(tab.XX) + IIf(lang == language.VisualBasic, "If my" + pClass.Name.Capitalized + ".ID = -1 Then", "if (my" + pClass.Name.Capitalized + ".ID == -1)").ToString());
        if (lang == language.CSharp)
            strB.AppendLine(Space(tab.XX) + "{");
    ...
    private string getUpdateObject(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            strB.AppendLine(Space(tab.X) + "Private Function update" + pClass.Name.Capitalized + "(ByVal my" + pClass.Name.Capitalized + " As " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized + ") As Boolean");
        ...
        {
            strB.AppendLine(Space(tab.X) + "private bool update" + pClass.Name.Capitalized + "(" + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized + " my" + pClass.Name.Capitalized + ")");
            strB.AppendLine(Space(tab.X) + "{");
    ...
    public string getViewCodeBehind(ProjectClass pClass, language lang)
    {
        ...
        if (lang == language.VisualBasic)
        {
            ...
            strB.AppendLine(Space(tab.XX) + "Dim listOfObjects As List(Of " + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized
        ...
        else
        {
            ...
            strB.AppendLine(Space(tab.XX) + "List<" + pClass.NameSpaceVariable.NameBasedOnID + "." + pClass.Name.Capitalized
        ...
        strB.AppendLine(Space(tab.XXX) + "tRow.Controls.Add(TableToolkit.getHyperlinkCell(AliasGroup.Edit.Capitalized, \"Edit"
        ...
        strB.AppendLine(Space(tab.XXX) + "tRow.Controls.Add(TableToolkit.getNoResultsFoundCell(AliasGroup." + pClass.Name.Capitalized
    ...
    {
        StringBuilder strB = new StringBuilder();
        strB.AppendLine(Space(tab.X) + "<asp:Hyperlink ID=\"hypAdd" + pClass.Name.Capitalized + "\" runat=\"server\" NavigateUrl=\"Edit"
    ...
    private string generateEditBody(ProjectClass pClass, bool useLists, language lang)
    {
        ...
        foreach (ClassVariable classVar in pClass.ClassVariables)
        {
            ...
            retStrB.Append(Space(tab.XXX) + "<asp:Label ID=\"lbl" + IIf(classVar.Name.ToLower().CompareTo("subtitle") == 0, pClass.Name, "").ToString()
            ...
            else if (classVar.IsDate)
            {
                ...
                retStrB.AppendLine(Space(tab.XXXX) + "<asp:TextBox ID=\"" + classVar.GetMonthTextControlName()
                                   + "\" runat=\"server\" CssClass=\"number\"></asp:TextBox>");
                retStrB.AppendLine(Space(tab.XXXX) + "<asp:TextBox ID=\"" + classVar.getDayTextControlName()
                                   + "\" runat=\"server\" CssClass=\"number\"></asp:TextBox>");
                retStrB.AppendLine(Space(tab.XXXX) + "<asp:TextBox ID=\"" + classVar.getYearTextControlName()
                                   + "\" runat=\"server\" CssClass=\"number\"></asp:TextBox>");
        ...
        retStrB.AppendLine(Space(tab.XXX) + rowBtnOpenTag + "<asp:Button ID=\"btnSaveChanges\" runat=\"server\" Text=\"Save Changes\" "
                           + IIf(lang == language.CSharp, "OnClick=\"btnSaveChanges_Click\" ", "").ToString() + "/>" + rowBtnCloseTag);
        retStrB.AppendLine(Space(tab.XXX) + rowBtnOpenTag + "<asp:Button ID=\"btnCancel\" runat=\"server\" Text=\"Cancel\" "
